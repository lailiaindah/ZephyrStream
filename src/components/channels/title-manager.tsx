...
export function TitleManager({ channelId, channelName }: TitleManagerProps) {
  ...
  const deleteAllMutation = useMutation({
    mutationFn: async () => {
      const res = await fetch(`/api/titles/delete-all?channelId=${channelId}`, { method: "DELETE" });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);
      return data;
    },
    ...
    <Card className="border-slate-800/60 bg-slate-900/40 overflow-hidden">
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent className="bg-slate-950 border-slate-800 text-white max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2"><Plus className="h-4 w-4 text-cyan-300" /> Add Title</DialogTitle>
            <DialogDescription className="text-slate-400">Add a new stream title for {channelName}</DialogDescription>
          </DialogHeader>
          <Input value={newTitle} onChange={(e) => setNewTitle(e.target.value)} placeholder="My Awesome Live Stream" autoFocus className="bg-slate-900 border-slate-700 text-white" />
          <DialogFooter>
            <Button variant="ghost" onClick={() => setAddOpen(false)}>Cancel</Button>
            <Button onClick={() => addMutation.mutate()} disabled={!newTitle.trim() || addMutation.isPending} className="bg-gradient-to-r from-cyan-500 to-emerald-500 text-slate-950">
              {addMutation.isPending && <Loader2 className="h-4 w-4 mr-2 animate-spin" />} Add Title
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={bulkOpen} onOpenChange={setBulkOpen}>
        <DialogContent className="bg-slate-950 border-slate-800 text-white max-w-lg">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2"><ListChecks className="h-4 w-4 text-cyan-300" /> Bulk Add Titles</DialogTitle>
            <DialogDescription className="text-slate-400">Paste one title per line. Empty lines are ignored.</DialogDescription>
          </DialogHeader>
          <Textarea value={bulkText} onChange={(e) => setBulkText(e.target.value)} placeholder={"Morning Live Stream\nAfternoon Gaming Session\nEvening Music Mix"} rows={8} autoFocus className="bg-slate-900 border-slate-700 text-white font-mono text-xs" />
          <DialogFooter>
            <Button variant="ghost" onClick={() => setBulkOpen(false)}>Cancel</Button>
            <Button onClick={() => bulkMutation.mutate()} disabled={!bulkText.trim() || bulkMutation.isPending} className="bg-gradient-to-r from-cyan-500 to-emerald-500 text-slate-950">
              {bulkMutation.isPending && <Loader2 className="h-4 w-4 mr-2 animate-spin" />} Add Titles
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
